...
def fs_tests():
    return [
        ...
        TestGroup('Reading file from rootdir', 'read', 1.5,
            ...
            Test('Reading from file partially', test_read_partial_root),
...
def run_cmd(args, allow_err=False):
    ...
    try:
        ...
        err += 'Timeout of %d seconds expired for command "%s"' % \
                (TIMEOUT, ' '.join(args))
...
def test_read_partial_root():
    """Read from a file where size is not the whole file (reading the whole
    file into the buffer would cause memory errors)."""
    fname = randpath()
    fconts = randstr(6 * 4096, 8 * 4096)
    with Filesystem((fname, fconts)) as fs:
        size = random.randrange(512 + 1, 1024 - 1)
        fs.check_pread(fname, size, 0)
